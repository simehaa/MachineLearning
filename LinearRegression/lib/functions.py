...
def ols_degree_and_n_analysis(
    ...
    * k=5: number of k subsets for k-fold CV
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax.set_title(
        f"OLS on Franke's function with \n" + "various number of data points.\n\n"
    )
    test_surf = ax.plot_surface(
        log_N_mesh, degree_mesh, mse_test_mesh, color="yellow", alpha=0.5
    )
    train_surf = ax.plot_surface(
        log_N_mesh, degree_mesh, mse_train_mesh, color="b", alpha=0.5
    )
    least_mse = ax.scatter(
        log_N_arr, least_MSE_degree, least_MSE_for_this_N, c="r", marker="o"
    )
    ...
    fake_test = mpl.lines.Line2D(
        [0], [0], linestyle="none", c="yellow", marker="s", alpha=0.8
    )
    fake_train = mpl.lines.Line2D(
        [0], [0], linestyle="none", c="b", marker="s", alpha=0.5
    )
    ax.legend(
        [fake_test, fake_train, least_mse],
        [
            f"MSE: {k}-fold CV",
            "MSE: Training data",
...
def lambda_and_degree_analysis(
    X_data, y, degree_arr, lambda_arr, method, variance, k=5
):
    """
    Info:
    ...
    * k=5: number of k subsets for k-fold CV
    ...
    ax.set_title(
        m
        + f" regression: MSE, evaluated with {k}-fold CV,\n"
...
def OLS_regression_on_terrain(filename, degree_arr, k=5, svd=False):
    ...
    * svd: option for matrix inversion by SVD
    ...
    ax.set_title(
        "OLS regression: MSE, evaluated {k}-fold CV" + f" with {k} subsets", loc="left"
    )
...
def regression_on_terrain_ridge_lasso(filename, method, degree_arr, lambda_arr, k=5):
    ...
    * alpha: parameter for ridge/lasso
    ...
    if method == "ridge":
        ...
        method_title = "Lasso "
    ax.set_title(
        method_title + "regression: MSE, evaluated with " + f"{k}-fold CV", loc="left"
    )
    minimum = ax.scatter(least_deg, least_lam, least_mse, c="r", marker="o")
    test_surf = ax.plot_surface(
        degree_mesh,
