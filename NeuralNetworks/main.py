...
def main():
    ...
    NN_classification(X_train, y_train, X_test, y_test, [29, 22, 16, 8, 1], 1)
...
def NN_classification(X_train, y_train, X_test, y_test, layers, epochs):
    ...
    # NN.SGD(X_train, y_train, epochs=epochs)
...
def LogReg_classification(X_train, y_train, X_test, y_test, runs, epochs):
    ...
    print(f"\tAccuracy over {runs} runs : {np.mean(acc):2.2f} +/- {np.std(acc):2.2f} %\n")
